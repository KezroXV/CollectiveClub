...
export async function checkAndPromoteFirstAdmin(userId: string, shopId: string) {
  ...
  if (!existingAdmin) {
    ...
    console.log(`User ${userId} promoted to admin for shop ${shopId}`);
    return true;
  }
...
export async function promoteUserToAdmin(targetUserId: string, adminUserId: string, shopId: string) {
  ...
  console.log(`User ${targetUserId} promoted to admin by ${adminUserId} for shop ${shopId}`);
  return updatedUser;
}
