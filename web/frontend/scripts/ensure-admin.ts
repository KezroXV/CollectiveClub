...
async function analyzeAdminStatus(): Promise<AdminCheckResult[]> {
  ...
  for (const shop of shops) {
    ...
    results.push({
      ...
      hasAdmin,
      adminCount,
      adminUsers: shop.users,
      actions
    });

    // Affichage du statut
    const status = hasAdmin ? "✅" : "❌";
    const adminInfo = hasAdmin 
      ? `${adminCount} admin(s)` 
      : "AUCUN ADMIN";
...
async function createEmergencyAdmin(
  ...
  try {
    ...
    if (shop.users.length > 0) {
      return { success: false, error: `Shop already has ${shop.users.length} admin(s)` };
    }
...
async function promoteToAdmin(
  ...
  try {
    ...
    if (requesterId) {
      ...
      if (!requester || requester.role !== 'ADMIN' || requester.shopId !== user.shopId) {
        return { success: false, error: `Requester lacks admin privileges in this shop` };
      }
    }
...
async function autoRepairOrphanedShops(): Promise<void> {
  ...
  console.log(`🚨 ${orphanedShops.length} boutique(s) sans administrateur détectée(s)`);
...
`;
    ...
    if (shop.adminUsers.length > 0) {
      ...
      for (const user of shop.adminUsers) {
        report += `  - ${user.email} (${user.role}) - Créé le ${user.createdAt.toLocaleDateString()}\n`;
      }
    }
  ...
  try {
    switch (command) {
      ...
      case 'create-admin':
        ...
        if (!shopId) {
          console.log('Usage: npm run ensure-admin create-admin <shopId> [email] [name]');
          process.exit(1);
        }
      ...
      case 'promote':
        ...
        if (!userId) {
          console.log('Usage: npm run ensure-admin promote <userId> [requesterId]');
          process.exit(1);
        }
